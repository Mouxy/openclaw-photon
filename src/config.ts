...
const PhotonAccountSchema = z
  .object({
    ...
    inboundBatching: z.boolean().optional().default(true),
    inboundBatchDelayMs: z.number().int().nonnegative().optional().default(2_500),
    inboundBatchMaxDelayMs: z.number().int().positive().optional().default(8_000),
    longTurnNotice: z.boolean().optional().default(false),
    longTurnNoticeDelayMs: z.number().int().positive().optional().default(45_000),
...
export function resolveAccount(cfg: any, accountId?: string | null): ResolvedPhotonAccount {
  ...
  return {
    ...
    inboundBatching: raw.inboundBatching ?? true,
    inboundBatchDelayMs: raw.inboundBatchDelayMs ?? 2_500,
    inboundBatchMaxDelayMs: raw.inboundBatchMaxDelayMs ?? 8_000,
    longTurnNotice: raw.longTurnNotice ?? false,
    longTurnNoticeDelayMs: raw.longTurnNoticeDelayMs ?? 45_000,
